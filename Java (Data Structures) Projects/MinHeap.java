...
public class Heap<V> {
     ...
     *      For each k, c[2k+1] and c[2k+2] are the left and right children of
     *      c[k].
     *      If k != 0, c[(k-1)/2] (using integer division) is the parent of
     *      c[k].
     *
    ...
    public void add(V v, double p) throws IllegalArgumentException {
        // TODO #1: Write this whole method. Note that bubbleUp is not
    	// implemented, so calling it will have no effect (yet). The first tests
    	// of add, using test00Add, ensure that this method maintains fields c
    	// and valPos properly, without worrying about bubbling up. Look at the
    	// spec of test00Add.
    	if(valPos.get(v) != null){
    		throw new IllegalArgumentException();
    	}
      Info c1 = new Info(v,p);
    	size = size + 1;
      fixSpace();
    	c[size-1] = c1;
    	valPos.put(v,size-1);
    ...
    private void bubbleUp(int k) {
        ...
        assert 0 <= k  && k < size;
        while ((k > 0) && (c[(k-1)/2].priority > c[k].priority)) {
            swap((k-1)/2, k);
            k = (k-1)/2;
        }
    }
    ...
    private void bubbleDown(int k) {
        ...
        // bubble down
        while ((2*k+2) <= size) {
            int smallerChild = 2*k+1;
            if ((2*k+3) <= size && c[2*k+1].priority > c[2*k+2].priority) {
                smallerChild = 2*k+2;
            }
    ...
    public void updatePriority(V v, double p) {
        ...
        Integer index = valPos.get(v);
    	if(index != null) {
    		c[index].priority = p;
    		if(index > 0 && c[(index-1)/2].priority > c[index].priority) {
    			bubbleUp(index);
    		}
    		else {
    			bubbleDown(index);
